...
def predict():
    try:
        ...
        if not data:
            return jsonify({
                "error": "Body deve ser um JSON válido."
            }), 400
        ...
        campos_nulos = [
            ...
            or entrada[col] == ""
            or (
                isinstance(entrada[col], float)
                and pd.isna(entrada[col])
            )
        ]
        ...
        # Encoder das categorias
        for col in CAT_COLS:
            encoder = encoders[col]
            valor = entrada[col]

            if valor not in encoder.classes_:
                valor = VALOR_MAP.get(valor)

            if valor is None or valor not in encoder.classes_:
                entrada[col] = 0
            else:
                entrada[col] = int(
                    encoder.transform([valor])[0]
                )
        ...
        try:
            prob = round(
                float(model.predict_proba(X_novo)[0][1]),
                4
            )
        except Exception:
            prob = None
